...
uN = x*0; vN = avg(x)*0;
uS = x*0; vS = avg(x)*0;
uW = avg(y)*1; %uW(floor(7*nx/8):floor(9*nx/10)) = uW(y)*1;
vW = y*0; %vW(floor(7*nx/8):floor(9*nx/10)) = y(floor(7*nx/8):floor(9*nx/10))*0+1; 
uE = avg(y)*1; %uE((floor(1*nx/10):floor(1*nx/8))) = avg(y)*1;
vE = y*0; %vE(floor(1*nx/10):floor(1*nx/8)) = y(floor(1*nx/10):floor(1*nx/8))*0-1;
%-----------------------------------------------------------------------
...
% treat nonlinear terms
    ...
    clf, contourf(avg(x),avg(y),P',20,'w-'); colorbar; hold on
    contour(x,y,Q',20,'k-');
